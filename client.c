...
int main(void)
{
	...
	printf("---------------------------------------------------------------------------------------------");
	printf(".................................  welcome to e-polling   ...................................");
	printf("---------------------------------------------------------------------------------------------");

	//Get Input from the User of name and cnic
	...
	//Receive the welcome message back from the server		
	if(recv(socket_desc, server_message, sizeof(server_message),0) < 0)
	{
		printf("Receive Failed. Error!!!!!\n");
		return -1;
	...
	if (strcmp(server_message, "Sorry! Your name is not in the authorized voters list.\n") != 0)
	{
	    ...
	    memset(server_message,'\0',sizeof(server_message));
	    memset(client_message,'\0',sizeof(client_message));				
        //just sending dummy conformation from user
	    ...
	    memset(server_message,'\0',sizeof(server_message));
	    memset(client_message,'\0',sizeof(client_message));
		//Receive the cadidates and symbols back from the server		
		if(recv(socket_desc, server_message, sizeof(server_message),0) < 0)
		{
			printf("Receive Failed. Error!!!!!\n");
			return -1;
		...
		if (strcmp(server_message, "Sorry! You have already casted a vote. You can't vote again.\n") != 0)
		{
			...
			//Receive end confirmation from server		
			if(recv(socket_desc, server_message, sizeof(server_message),0) < 0)
			{
				printf("Receive Failed. Error!!!!!\n");
				return -1;
